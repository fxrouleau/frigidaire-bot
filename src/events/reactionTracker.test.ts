...
describe('reactionTracker event', () => {
  ...
  it('increments use_count for a custom emoji reaction', () => {
    ...
    reactionEvent.execute(fakeReaction('123456', 'pog'), fakeUser(false), DETAILS);

    expect(useCount('123456')).toBe(1);
  });

  it('skips Unicode emoji reactions (null emoji id) without error', () => {
    expect(() => reactionEvent.execute(fakeReaction(null, '🔥'), fakeUser(false), DETAILS)).not.toThrow();
  ...
  it('skips reactions added by a bot user', () => {
    ...
    reactionEvent.execute(fakeReaction('777', 'botreact'), fakeUser(true), DETAILS);

    expect(useCount('777')).toBe(0);
  });

  it('does not throw when the custom emoji is not tracked', () => {
    expect(() => reactionEvent.execute(fakeReaction('888888', 'ghost'), fakeUser(false), DETAILS)).not.toThrow();

...
- **Birthdays** (bot.db `birthdays`): `set_birthday` (`MM-DD` or `YYYY-MM-DD`), `list_birthdays` (by next occurrence, with the age they'll turn) and `forget_birthday`.
  ...
  - The text is written by `CHAT_MODEL` (ZDR, low reasoning effort, tagged `birthday`) with ≤5 of the person's memories, then cleaned and 🎂-prefixed. Those memories are long-standing ones: no `event`, `image` or self-diagnosis rows, and nothing created in the last 14 days (a story from yesterday made a poor birthday line), most recently confirmed first; the prompt asks for a trait, a running joke or old lore, never recent news. A template is used on failure.
